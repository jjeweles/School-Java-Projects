...
public class PizzaControls extends JPanel {
	...
	public PizzaControls() {
		...
		pnlNameAndPizza.setLayout(new BoxLayout(pnlNameAndPizza, BoxLayout.X_AXIS));
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (!gotName) {
				...
				if (!text.equals("")) {
					output1 = output1 + "Order for " + text + ": ";
					gotName = true;
				} else { // didnt enter a customers name error
					JOptionPane.showMessageDialog(null, "Error! You must enter customer's name.",
			...
			if (gotName) {
				...
				// find out pizza size selection
				if (selection == 0) { // large selected

					output1 = output1 + pizzaSize[0] + " ";
					gotLarge = true;

				} else if (selection == 1) { // medium selected

					output1 = output1 + pizzaSize[1] + " ";
					gotMedium = true;

				} else if (selection == 2) { // small selected
	...
	private class PizzaStyleListener implements ActionListener {
		public void actionPerformed(ActionEvent event) {
			...
			if (!gotSize) {
				JOptionPane.showMessageDialog(null, "Error! You must select the size of the pizza!", "Pizza size error",
						JOptionPane.ERROR_MESSAGE);
			} else {
				selection = cboPizzaStyle.getSelectedIndex();

				switch (selection) {
				case 0: // if hand tossed
					output1 = output1 + pizzaStyle[0] + " pizza with\n";
					gotHandTossed = true;
					break;
				case 1: // if thin crust
					output1 = output1 + pizzaStyle[1] + " pizza with\n";
					gotThin = true;
					break;
				case 2: // if deep dish
					output1 = output1 + pizzaStyle[2] + " pizza with\n";
					gotDeepDish = true;
					break;
				default:
					output1 = output1 + "Unknown style!";
				}
	...
	private class CompleteListener implements ActionListener {
		public void actionPerformed(ActionEvent event) {
			...
			if (transactionType == false) {
				System.out.println("You must choose a " + "transaction type!");
				// display error message; you must have entered
				// chosen a transaction type first
				JOptionPane.showMessageDialog(null, "Error! You must choose a " + "transaction type!",
						"Transaction type error", JOptionPane.ERROR_MESSAGE);
			} else // transaction type has been selected
			{
				...
				if (completeOrder == false) {
					...
					gatherToppings1();
					gatherToppings2();
					lblCost.setText("Total Cost: " + fmt2.format(totalCost));
	...
	private class ChangeListener implements ActionListener {
		public void actionPerformed(ActionEvent event) {
			...
			// get what customer pays
			// and parse it into a double
			text = txtPaid.getText();
			paid = Double.parseDouble(text);

			// get amt of change owed
			change = paid - totalCost;

			// display change in formatted txt
	...
	private class Toppings1Listener implements ItemListener {
		public void itemStateChanged(ItemEvent event) {
			...
			// and update boolean of topping to true
	...
	private class Toppings2Listener implements ItemListener {
		public void itemStateChanged(ItemEvent event) {
			...
			// see which topping was selected
	...
	private class TransactionListener implements ActionListener {
		public void actionPerformed(ActionEvent event) {
			...
			// check to see what payment type was selected
			// add fifty cents if paying by check
	...
	private void getSizeCost() {
		...
		// determine size of pizza and add cost to total
		if (gotLarge) {
			totalCost += LG_COST;
		} else if (gotMedium) {
			totalCost += MD_COST;
	...
	private void getStyleCost() {
		...
		// determine style of pizza and add cost to total
		if (gotHandTossed) {
			totalCost += HT_COST;
		} else if (gotThin) {
			totalCost += TN_COST;
